    ...
    def _serve_device_replacement_check(self):
        ...
        try:
            ...
            if not device_serial:
                self._send_error(400, "Device serial number required in URL path")
                return
            ...
            CPU_LOW_THRESHOLD = 5.0        # Low CPU threshold (%) - indicates device may be underutilized
            MEMORY_HIGH_THRESHOLD = 85.0   # High memory threshold (%)
            MEMORY_LOW_THRESHOLD = 10.0    # Low memory threshold (%) - indicates potential issues
            TIME_WINDOW_MINUTES = 10       # Time window for averaging (minutes)
            PROMETHEUS_URL = "http://prometheus:9090"  # Prometheus URL (adjust if needed)
                ...
                if cpu_response.status_code == 200:
                    ...
                    if cpu_data.get("status") == "success" and cpu_data.get("data", {}).get("result"):
                        ...
                        if avg_cpu >= CPU_HIGH_THRESHOLD:
                            ...
                            reasons.append(f"High CPU usage: {avg_cpu:.1f}% (threshold: {CPU_HIGH_THRESHOLD}%)")
                        elif avg_cpu <= CPU_LOW_THRESHOLD:
                            replacement_needed = True
                            reasons.append(f"Unusually low CPU usage: {avg_cpu:.1f}% (threshold: {CPU_LOW_THRESHOLD}%)")
                ...
                if memory_response.status_code == 200:
                    ...
                    if memory_data.get("status") == "success" and memory_data.get("data", {}).get("result"):
                        ...
                        if avg_memory >= MEMORY_HIGH_THRESHOLD:
                            replacement_needed = True
                            reasons.append(f"High memory usage: {avg_memory:.1f}% (threshold: {MEMORY_HIGH_THRESHOLD}%)")
                        elif avg_memory <= MEMORY_LOW_THRESHOLD:
                            replacement_needed = True
                            reasons.append(f"Unusually low memory usage: {avg_memory:.1f}% (threshold: {MEMORY_LOW_THRESHOLD}%)")
            ...
            logger.info(f"{status_emoji} Device replacement check for {device_serial}: {replacement_needed}")
            if reasons:
    ...
    def _check_current_metrics_fallback(self, device_serial: str) -> bool:
        ...
        try:
            ...
            with self.registry._lock:
                ...
                memory_metrics = self.registry.metrics.get("total_memory_usage_percent", [])
                
                current_time = datetime.now()
                recent_threshold = timedelta(minutes=2)  # Only consider metrics from last 2 minutes
                
                # Check CPU
                for metric in cpu_metrics:
                    if (metric.labels.get("device_serial") == device_serial and 
                        current_time - metric.timestamp < recent_threshold):
    ...
    def _ingest_metrics(self):
        ...
        try:
            ...
            if not isinstance(data, dict) or 'metrics' not in data:
                self._send_error(400, "Invalid request format. Expected: {'metrics': [...]}")
                return
            ...
            logger.info(f"✅ Processed {processed_count} metrics from device {device_serial}")
...
def main():
    ...
    parser.add_argument("--ttl", type=int, default=300, help="Metric TTL in seconds (default: 300)")
